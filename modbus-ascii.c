...
int _modbus_ascii_prepare_response_tid(const uint8_t *req, int *req_length)
{
...
int _modbus_ascii_send_msg_pre(uint8_t *req, int req_length)
{
...
ssize_t _modbus_ascii_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
#if defined(_WIN32)
    modbus_ascii_t *ctx_ascii = ctx->backend_data;
    DWORD n_bytes = 0;
    return (WriteFile(ctx_ascii->w_ser.fd, req, req_length, &n_bytes, NULL)) ? n_bytes : -1;
#else
    return write(ctx->s, req, req_length);
#endif
}

ssize_t _modbus_ascii_recv(modbus_t *ctx, uint8_t *rsp, int rsp_length)
{
#if defined(_WIN32)
...
int _modbus_ascii_flush(modbus_t *);
...
int _modbus_ascii_check_integrity(modbus_t *ctx, uint8_t *msg,
...
static int _modbus_ascii_connect(modbus_t *ctx)
{
    ...
    ctx->s = open(ctx_ascii->device, O_RDWR | O_NOCTTY | O_NDELAY | O_EXCL);
    if (ctx->s == -1) {
        fprintf(stderr, "ERROR Can't open the device %s (%s)\n",
                ctx_ascii->device, strerror(errno));
        return -1;
...
void _modbus_ascii_close(modbus_t *ctx)
{
    /* Closes the file descriptor in ASCII mode */
    modbus_ascii_t *ctx_ascii = ctx->backend_data;

#if defined(_WIN32)
    /* Revert settings */
    if (!SetCommState(ctx_ascii->w_ser.fd, &ctx_ascii->old_dcb))
        fprintf(stderr, "ERROR Couldn't revert to configuration (LastError %d)\n",
                (int)GetLastError());

    if (!CloseHandle(ctx_ascii->w_ser.fd))
...
int _modbus_ascii_flush(modbus_t *ctx)
{
#if defined(_WIN32)
    modbus_ascii_t *ctx_ascii = ctx->backend_data;
...
int _modbus_ascii_select(modbus_t *ctx, fd_set *rfds,
...
int _modbus_ascii_filter_request(modbus_t *ctx, int slave)
{
                         ...
                         int stop_bit)
{
    ...
    dest_size = sizeof(ctx_ascii->device);
    ret_size = strlcpy(ctx_ascii->device, device, dest_size);
    if (ret_size == 0) {
